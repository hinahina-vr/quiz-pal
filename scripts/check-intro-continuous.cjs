...
(async()=>{
 ...
 try{
  ...
  for(const [width,height] of viewports){
   ...
   await expect(modal.locator('[data-reading-section]')).toHaveCount(5);
   ...
   await jump('conversation','[data-step="1"]');await jump('images','[data-action=next]');await jump('choose','[data-action=next]');await expect(modal.locator('[data-step="1"]')).toHaveAttribute('aria-current','step');
   for(const provider of ['openrouter','openai','gemini','grok']){
    await jump('setup',`[data-provider=${provider}]`);await expect(modal.locator('[data-reading-section]')).toHaveCount(6);await expect(modal.locator('#productIntroProviderDetail')).toContainText(provider==='openai'?'GPT':provider==='gemini'?'Gemini':provider==='grok'?'Grok':'OpenRouter');await jump('choose','[data-action=choose-provider]');
   }
   await jump('images','[data-feature=images]');await expect(modal.locator('[data-feature=images]')).toHaveAttribute('aria-pressed','true');await expect(modal.locator('[data-step="1"]')).toHaveAttribute('aria-current','step');
   ...
   await jump('conversation','[data-step="1"]');await modal.locator('[data-capture-step="ai-first-answer.png"]').click();await expect(modal.locator('[data-reading-section=conversation] [data-screenshot] img')).toHaveAttribute('src','./guide-captures/ai-first-answer-preview.webp');
   ...
   await body.evaluate(e=>e.scrollTop=0);await expect.poll(()=>modal.getAttribute('data-reading-section')).toBe('overview');
